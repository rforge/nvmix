...
pStudent <- function(upper, lower = rep(-Inf, length(upper)), shift = rep(0, length(upper)), scale, df, standardized = FALSE, gam = 3.3, abserr = 0.001, Nmax = 1e8, N = 12, n_init = 2^5, precond = TRUE, method = "sobol")
{
  ...
  ## Deal with df = Inf
  if(!is.finite(df)){
    ## Note that we already standardized and subtracted the shift. 
    return(pmultinorm(upper = upper, lower = lower, scale = scale, standardized = TRUE, gam = gam, abserr = abserr, Nmax = Nmax,
           N = N, n_init = n_init, precond = precond, method = method))
  }
  ...
  while(err > abserr && N. < Nmax)
  {
    ...
    if(i. == 0){
      ...
      for(l in 1:N){
        ...
        U <- switch(method,
                    "sobol"   = {
                      qrng::sobol(n = n., d = q, randomize = TRUE, skip = 0)
                    },
                    "gHalton" = {
      ...
      for(l in 1:N){
        ...
        U <- switch(method,
                    "sobol"   = {
                      qrng::sobol(n = n., d = q, randomize = TRUE, skip = n.)
                    },
                    "gHalton" = {
                      qrng::ghalton(n = n., d = q, method = "generalized")
                    },
