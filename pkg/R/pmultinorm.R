...
##' @param mean mean vector, in this case the mean
...
##' @author Erik Hintz
pmultinorm <- function(upper, lower = rep(-Inf, length(upper)), mean = rep(0, length(upper)), scale, standardized = FALSE, gam = 3.3, abserr = 0.001, Nmax = 1e8, B = 12, n_init = 2^6, precond = TRUE, method = "sobol")
{
  pnvmix(upper = upper, lower = lower, loc = mean, scale = scale, mix = "constant", standardized = standardized, gam = gam, abserr = abserr, Nmax = Nmax, B = B, n_init = n_init, precond = precond, method = method)
}