...
fitnvmix <- function(X, qmix,
                     ...
                     laststep.do.nu = !ECMEstep.do.nu,
                     fitting.method = c("cScov", "CopX", "CopU"),
                     ECME.maxiter = 25, conv.tol = 1e-2,
                     control.optim = list(), control = list(),
                     conv.tol.nu = 1e-2, verbose = TRUE)
{
  ...
  if(fitting.method != "cScov"){
    ...
    neg.log.likelihood.init.nu <- function(nu){
      if(inv.gam){
        - sum(dnvmixcop(U, qmix = "inverse.gamma", factor = chol.P, control = control,
                        verbose = verbose, log = TRUE, df = nu))
      } else {
    ...
    ## Estimate 'scale' as multiple of SCov; choose the factor to max loglikelihood
    neg.log.likelihood.c <- function(c){
      if(inv.gam){
        - sum(dnvmixcop(U, qmix = "inverse.gamma", factor = sqrt(c)*chol.P, control = control,
                        verbose = verbose, log = TRUE, df = nu.est))
      } else {
        ...
        ldens.obj <- dnvmix.int(qW = qmix., maha2.2 = maha2.2/c, lrdet = (lrdet + d/2*log(c)),
    ...
    start.c <- mean(qW(sobol(n = 100, d = 1, randomize = TRUE), nu = nu.est))
    ...
    neg.log.likelihood.init <- function(param){
      if(inv.gam){
        ...
        ## Call dnvmix.int which by default returns the log-density
        ldens.obj <- nvmix:::dnvmix.int(qW = qmix., maha2.2 = maha2.2/c, lrdet = (lrdet + d/2*log(c)),
                                        U0 = U0, d = d, method = method, abstol = dnvmix.abstol,
                                        CI.factor = CI.factor, fun.eval = fun.eval, max.iter.rqmc = max.iter.rqmc,
                                        B = B, seed = seed)
        ## Check if error tolerance reached
  ...
  if(ECMEstep){
    ...
    get.weights <- function(nu, loc, scale, U, abstol = control$weights.abstol){
      ...
      error <- CI.factor * max( sd( rqmc.estimates.density[, 1]), # smallest maha index in column 1
      ...
      while(error > abstol && total.fun.evals < fun.eval[2]){
        ...
        error <- CI.factor * max( sd( rqmc.estimates.density[, 1]),
                                  sd( rqmc.estimates.condexp[, 1]))
      }
      ## Return
    ...
    while(num.iter < ECME.maxiter && !converged){
      ...
      if(!converged.nu && ECMEstep.do.nu){
        if(method != "cScov"){
          ## Correlation matrix:
          P <- cov2cor(scale.est)
          chol.P <- t(chol(P))
          ...
          ## Optimize neg.log.likelihood over nu
          nu.est <- optim(nu.est, fn = neg.log.likelihood.nu,
                          lower = mix.param.bounds.lower,
                          upper = mix.param.bounds.upper,
                          method = "L-BFGS-B", control = control.optim)$par
        } else {
          ## Determine new maha distance and determinant of scale factor:
          factor.est <- t(chol(scale.est)) 
      ...
      converged <- (abs(diff.nu) < conv.tol.nu) &&
