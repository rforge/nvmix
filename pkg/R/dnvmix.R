...
dnvmix.extrapolate <- function(maha2.2, ldensities, errors, control){
   n <- length(maha2.2)
   ## Find first index where 'abstol' was not reached:
   fnotrchd <- which.max( (errors >= control$dnvmix.abstol))
   ...
   ind.test <- if(fnotrchd + control$dnvmix.extrap.num.test - 1 <= n){
      ...
      ## If not, just take the last 'dnvmix.extrap.num.test' ones
   ...
   log.approx <- function(maha, dens, linear = FALSE){
      err <- if(linear){
         ...
         start <- c(-1, 0, 1, 1, -1, 1)
         low <- c(-Inf, -Inf, 0, 0 , -Inf, -Inf)
         upp <- c(0, 0, Inf, Inf, Inf, Inf)
         function(coeff){
            sum((coeff[1] + coeff[2]*log(1e-12 + coeff[3] + coeff[4]*maha)
