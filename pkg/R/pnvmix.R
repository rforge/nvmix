...
##' @param abserr numeric >= 0 providing the absolute precision required.
##'        If abserr = 0, algorithm will run until total number of function
##'        evaluations exceeds Nmax.
##' @param CI.factor Monte Carlo confidence interval multiplier. Algorithm runs
##'        CI.factor * (estimated standard error) < abserr. If CI.factor = 3.3
##'        (default), one can expect the actual absolute error to be less than
##'        abserr in 99.9% of the cases
##' @param max.fun.eval maximal number of function evaluations
