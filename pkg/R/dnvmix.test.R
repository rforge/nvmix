...
dnvmix.int.old <- function(qW, maha2.2, lrdet, d, control, verbose)
{
  ## 1 Basics ##################################################################
    ...
    for(b in 1:B){
      ...
      rqmc.estimates[b,] <-
        if(dblng) {
          ...
          logsumexp(rbind(rqmc.estimates[b,], next.estimate, deparse.level = 0)) - log(denom)
          ...
          logsumexp(rbind(log(numiter) + rqmc.estimates[b, ], next.estimate, deparse.level = 0)) - 
            log(numiter + 1)
...
##' @author Erik Hintz and Marius Hofert
dnvmix.old <- function(x, qmix, loc = rep(0, d), scale = diag(d),
                   ...
                   control = list(), log = FALSE, verbose = TRUE,...)
{
  ...
  if(inv.gam) { # multivariate t
    ...
    ## Prepare inputs for dnvmix.int.old
    ...
    ## Note that 'dnvmix.int.old' calls 'dnvmix.extrapolate' (if necessary)
    ests <- dnvmix.int.old(qW, maha2.2 = maha2.2, lrdet = lrdet, d = d,
