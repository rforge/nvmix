...
#     rqmc.strat.obj <- dnvmix.internal.adaptRQMC(qW, maha2.2[notRchd], lrdet = lrdet, d = d,
...
#   ## Note: W[1] = smallest W we can generate; W[Bn0p2] largest W we can generate
...
#       candid.right <- if(ind.greater[length(ind.greater)] < Bn0p2){
...
#     ## 5.1 Estimate log of integral over (0, u.left) #############################
...
#         upper.sum <- logsumexp.matrix( 
#           as.matrix(log(weights) + l.integrand[1:numObs], ncol = 1))
#         lower.sum <- logsumexp.matrix( 
...
#           upper.sum <- logsumexp.matrix( 
#             as.matrix(log(weights) + l.integrand[1:lastUsed], ncol = 1))
#           lower.sum <- logsumexp.matrix( 
#             as.matrix(log(weights) + c(-Inf, l.integrand[1:(lastUsed-1)]), ncol = 1))
#           (lower.sum + upper.sum) / 2
#         } else {
...
#       ldens.left
#     }
#     ## 5.2 Estimate log of integral over (u.right, 1) ############################
...
#           upper.sum <- logsumexp.matrix( 
#             as.matrix(log(weights) + l.integrand[firstUsed:numObs], ncol = 1))
#           lower.sum <- logsumexp.matrix( 
#             as.matrix(log(weights) + c(l.integrand[(firstUsed+1):numObs], -Inf), ncol = 1))
#           (lower.sum + upper.sum) / 2
#         } else {
#           ## Case 2: No observations in (u.right, 1) => u.right > ONE 
...
#       ldens.right 
#     } 
#     ## 5.3 Estimate log of integral over (u.left, u.right) #######################
...
#         ldens.obj <- nvmix:::dnvmix.int.t(qW, maha2.2 = curr.maha2.2, lrdet = lrdet, d = d, 
#                                           control = control, tol = tol, do.reltol = do.reltol,
#                                           lower.q = u.left, upper.q = u.right, verbose = verbose)
...
#     ldensities[ind] <- logsumexp.matrix(rbind(ldens.left, ldens.right, 
#                                               ldens.stratum, deparse.level = 0)) 
#     errors[ind]    <- error
