...
pnvmix(b, lower = a, qmix = function(u, lambda) -log(1-u)/lambda,
       lambda = rate, scale = P, control = list(abstol = 1e-5))
```
...
argument. A few of them are (for others see `?pnvmix`):
...
for(i in seq_along(max.fun.evals)) {
    ...
    errors[nms[1],i] <-
        attr(pStudent(b, lower = a, scale = P,  df = df,
                      control = list(abstol = NULL, fun.eval = c(2^6, N.max)),
                      verbose = FALSE), "error")
    ## Sobol without preconditioning
    set.seed(42)
    errors[nms[2],i] <-
        attr(pStudent(b, lower = a, scale = P,  df = df,
                      control = list(precond = FALSE, abstol = NULL, 
    ...
    errors[nms[3],i] <-
        attr(pStudent(b, lower = a, scale = P,  df = df,
                      control = list(method = "PRNG", abstol = NULL, 
                                     fun.eval = c(2^6, N.max)), 
                      verbose = FALSE), "error")
    ...
    errors[nms[4],i] <-
        attr(pStudent(b, lower = a, scale = P,  df = df,
                      control = list(method = "PRNG", precond = FALSE, 
                                     abstol = NULL, fun.eval = c(2^6, N.max)),
                      verbose = FALSE), "error")
}
