...
the (lower triangular) Cholesky factor $A$ of $\Sigma$ such that $AA^\top=\Sigma$.
...
of the form
\begin{align*}
	F(\mathbf{a},\mathbf{a} )=P(\mathbf{a} < \mathbf{X}\le\mathbf{b})=P(a_1 < X_1\le b_1,\dots, a_d < X_d\le b_d),\quad \mathbf{a},\mathbf{b}\in\mathbb{R}^d
\end{align*}
where $\mathbf{a}<\mathbf{b}$ (interpreted componentwise) where entries of 
...
accomplished by changing the argument `pnvmix.abstol` (which defaults to `1e-3`) at the
...
argument. A few of them are (for others see `?get.set.parameters`):
...
variable $W$ is available. Internally, an EMCE (Expectation Maximization Constraint/Either)
algorithm is used to estimate the mixing parameters of $W$, the location 
...
scale     <- diag_vars %*% cov2cor(A %*% t(A)) %*% diag_vars # true scale matrix
nu        <- 3 # true mixing parameter 
mix.param.bounds  <- c(1, 7) # nu in [1, 5]
