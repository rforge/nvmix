...
As a next example, consider a normal variance mixture where $W$ is discrete. For instance, let $W$ such that $P(W=1)=0.5$, $P(W=5)=0.1$ and $P(W=10)=0.4$. We use the same parameters as above, but this time, we are interested in the one sided probabability $P( \mathbf{X} \leq \mathbf{b})$:

```{r}

## Define quantile function.
qW <- function(u)
  (u <= 0.5) * 1 + ( u > 0.5 & u <= 0.6) * 5 + ( u > 0.6 ) * 10
...
as.numeric( 0.5 * pNorm(b, scale = P) + 0.1 * pNorm(b, scale = 5*P) +
  0.4 * pNorm(b, scale = 10*P) )
